  ...
  setSearchParams(searchParams: SearchParams) {
    ...
    this.router.navigate([], {
      queryParams: {
        ...
        fs: searchParams.filter_audiences?.map(x => x.slice('human_situations:'.length)).join('|') || null,
        ...
        fh: searchParams.filter_health_issues?.map(x => x.slice('human_situations:'.length)).join('|') || null,
